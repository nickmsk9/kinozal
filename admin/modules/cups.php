    ...
    function CupsAdmin()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_cups'])) {
            ...
            foreach (cups_catalog() as $cup) {
                ...
                if ($username === '') {
                    cups_release($cup_id, 'manual');
                    continue;
                }

                $user = cups_find_user_by_username($username);

                if (!$user) {
                    $messages[] = 'Пользователь "' . cups_h($username) . '" не найден для кубка "' . cups_h($cup['title']) . '".';
                    continue;
                }

                cups_assign($cup_id, (int)$user['id'], 'manual', 0, (int)$CURUSER['id'], 'Назначено из админки');
                $messages[] = cups_h($cup['title']) . ': назначен пользователю ' . cups_h($user['username']) . '.';
            }
        }

        if (!empty($_GET['force']) || (isset($_POST['force_update']) && $_POST['force_update'])) {
            cups_update_auto(true);
        ...
        echo '<p><a href="' . cups_h($admin_file) . '.php?op=CupsAdmin&amp;force=1">Обновить авто-кубки сейчас</a></p>';
        echo '<form method="post" action="' . cups_h($admin_file) . '.php?op=CupsAdmin">';
        ...
        foreach ($current as $row) {
            ...
            echo '<td><b><i class="i1 ' . cups_h($row['icon']) . '"></i> ' . cups_h($row['title']) . '</b></td>';
            echo '<td>' . $current_user . '</td>';
            echo '<td class="center">' . $source . '</td>';
            echo '<td><input type="text" name="cup_user[' . $cup_id . ']" value="' . cups_h($manual_value) . '" size="35"> <span class="small">пусто = снять ручное назначение</span></td>';
            echo '</tr>';
        }
