...
require_once 'include/group_page.php';
...
function GroupPageAdmin()
{
	...
	if ($message !== '') {
		stdmsg('Группы', group_page_h($message));
	}

	$items = group_page_rows(false);
	...
	echo '<form method="post" action="' . group_page_h($admin_file) . '.php?op=GroupPageAdmin">';
	...
	foreach ($items as $row) {
		...
		echo '<td><a href="/groupex.php?id=' . $group_id . '">' . group_page_h($row['name']) . '</a> <span class="small">ID ' . $group_id . '</span></td>';
		echo '<td class="center">' . group_page_h(groups_type_name((int)$row['type'])) . '</td>';
	...
	while ($group = mysqli_fetch_assoc($groups_res)) {
		echo '<option value="' . (int)$group['id'] . '">' . group_page_h($group['name']) . ' / ID ' . (int)$group['id'] . ' / ' . group_page_h(groups_type_name((int)$group['type'])) . '</option>';
	}
	echo '</select></td></tr>';
