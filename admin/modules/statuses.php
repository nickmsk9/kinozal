...
if (!function_exists('UserStatusesAdmin')) {
	function UserStatusesAdmin()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_status_settings'])) {
			...
			foreach (statuses_catalog() as $key => $status) {
		...
		if ($lookup_name !== '') {
			$selected_user = statuses_find_user_by_username($lookup_name);
			if (!$selected_user) {
				$messages[] = 'Пользователь "' . statuses_h($lookup_name) . '" не найден.';
			}
		}
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_user_statuses']) && $selected_user) {
			...
			statuses_save_manual((int)$selected_user['id'], $selected, (int)$CURUSER['id']);
			$messages[] = 'Статусы пользователя ' . statuses_h($selected_user['username']) . ' сохранены.';
		}
		...
		echo '<form method="post" action="' . statuses_h($admin_file) . '.php?op=UserStatusesAdmin">';
		...
		foreach ($statuses as $key => $status) {
			...
			echo '<tr>';
			echo '<td class="center"><input type="checkbox" name="active[' . statuses_h($key) . ']" value="1"' . $checked . '></td>';
			echo '<td><i class="i1 ' . statuses_h($status['icon_class']) . '"></i> <b>' . statuses_h($status['title']) . '</b></td>';
		...
		echo '<div class="tp1_body">';
		echo '<form method="get" action="' . statuses_h($admin_file) . '.php">';
		echo '<input type="hidden" name="op" value="UserStatusesAdmin">';
		echo '<table class="tables2 w100p"><tr><td class="w150">Пользователь</td><td><input type="text" name="username" size="35" value="' . statuses_h($lookup_name) . '"> <input type="submit" class="btn" value="Найти"></td></tr></table>';
		echo '</form>';

		if ($selected_user) {
			$manual = statuses_manual_keys((int)$selected_user['id']);
			echo '<form method="post" action="' . statuses_h($admin_file) . '.php?op=UserStatusesAdmin">';
			echo '<input type="hidden" name="save_user_statuses" value="1">';
			echo '<input type="hidden" name="username" value="' . statuses_h($selected_user['username']) . '">';
			echo '<table class="tables2 w100p">';
			echo '<tr><td colspan="2">Пользователь: <a href="/userdetails.php?id=' . (int)$selected_user['id'] . '">' . get_user_class_color((int)$selected_user['class'], statuses_h($selected_user['username'])) . '</a></td></tr>';
			...
			foreach ($statuses as $key => $status) {
				...
				echo '<tr><td class="w20 center"><input type="checkbox" name="manual_statuses[]" value="' . statuses_h($key) . '"' . $checked . '></td><td><i class="i1 ' . statuses_h($status['icon_class']) . '"></i> ' . statuses_h($status['title']) . '</td></tr>';
			}
