...
if (!function_exists('RadioAdmin')) {
	...
	{
		echo '<tr>';
		echo '<td class="rowhead w150">' . kz_radio_h($label) . '</td>';
		echo '<td><input type="text" name="' . kz_radio_h($name) . '" size="' . (int)$size . '" value="' . kz_radio_h($value) . '"></td>';
		echo '</tr>';
	}
	...
	function RadioAdmin()
	{
		...
		$chat = kz_radio_chat_messages(11, 20);
		$chat = array_merge($chat, kz_radio_chat_messages(12, 20));
		...
		echo '<form method="post" action="' . kz_radio_h($admin_file) . '.php?op=RadioAdmin">';
		...
		echo '<tr><td class="rowhead">Анонс</td><td><textarea name="announce_text" rows="5" cols="90">' . kz_radio_h($settings['announce_text']) . '</textarea></td></tr>';
		radio_admin_row('Кому отправлять анкету', 'recruit_contact', $settings['recruit_contact']);
		echo '<tr><td class="rowhead">Текст набора</td><td><textarea name="rules_text" rows="22" cols="90">' . kz_radio_h($settings['rules_text']) . '</textarea></td></tr>';
		...
		echo '<div class="tp1_body">';
		echo '<form method="post" action="' . kz_radio_h($admin_file) . '.php?op=RadioAdmin" onsubmit="return confirm(\'Очистить весь чат радио?\');">';
		...
		foreach ($chat as $row) {
			...
			echo '<td><a href="/userdetails.php?id=' . (int)$row['userid'] . '" class="u' . (int)$row['userclass'] . '">' . kz_radio_h($row['username']) . '</a></td>';
			echo '<td>' . nl2br(kz_radio_h($row['text'])) . '</td>';
			echo '<td>' . kz_radio_h($row['added']) . '</td>';
			echo '<td class="center"><form method="post" action="' . kz_radio_h($admin_file) . '.php?op=RadioAdmin" onsubmit="return confirm(\'Удалить сообщение?\');"><input type="hidden" name="delete_chat_id" value="' . (int)$row['id'] . '"><input type="submit" class="btn" value="Удалить"></form></td>';
			echo '</tr>';
		}
