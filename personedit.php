...
$person = $id > 0 ? persons_find($id, '') : null;

if ($id > 0 && !$person) {
	stderr($tracker_lang['error'], 'Персона не найдена.');
}

if (!persons_can_edit($person)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	persons_save_photos($id, $_POST['photos'] ?? '');
	header('Location: ' . str_replace('&amp;', '&', persons_url($name, $id)));
	exit;
}

$photos_text = $id > 0 ? persons_photo_text($id) : '';
...
<div class="mn_wrap">
	...
	<div class="bx1_0"><table class="tables1 w100p">
		<tr><td class="w175">Имя:</td><td><input type="text" name="name" class="w98p" value="<?= persons_h($person['name']) ?>"></td></tr>
		<tr><td>Оригинальное имя:</td><td><input type="text" name="original_name" class="w98p" value="<?= persons_h($person['original_name']) ?>"></td></tr>
		...
		<tr><td>Постер:</td><td><input type="text" name="poster_url" class="w98p" value="<?= persons_h($person['poster_url']) ?>"></td></tr>
		<tr><td>Фото:</td><td><textarea name="photos" rows="5" class="w98p" placeholder="Ссылки на фото, по одной в строке"><?= persons_h($photos_text) ?></textarea></td></tr>
		<tr><td>Дата рождения:</td><td><input type="text" name="birth_date" value="<?= persons_h($person['birth_date']) ?>" placeholder="YYYY-MM-DD"> <input type="text" name="birth_text" size="50" value="<?= persons_h($person['birth_text']) ?>" placeholder="Текстом, если нужно"></td></tr>
		<tr><td>Место рождения:</td><td><input type="text" name="birth_place" class="w98p" value="<?= persons_h($person['birth_place']) ?>"></td></tr>
		<tr><td>Карьера:</td><td><input type="text" name="career" class="w98p" value="<?= persons_h($person['career']) ?>"></td></tr>
		<tr><td>Жанр:</td><td><input type="text" name="genre" class="w98p" value="<?= persons_h($person['genre']) ?>"></td></tr>
		<tr><td>Рост:</td><td><input type="text" name="height" value="<?= persons_h($person['height']) ?>"></td></tr>
		<tr><td>Супруг(а):</td><td><input type="text" name="spouse" class="w98p" value="<?= persons_h($person['spouse']) ?>"></td></tr>
		<tr><td>Биография:</td><td><textarea name="biography" rows="12" class="w98p"><?= persons_h($person['biography']) ?></textarea></td></tr>
		<tr><td>Знаете ли Вы:</td><td><textarea name="trivia" rows="6" class="w98p"><?= persons_h($person['trivia']) ?></textarea></td></tr>
		<tr><td>Фильмография:</td><td><textarea name="filmography" rows="8" class="w98p"><?= persons_h($person['filmography']) ?></textarea></td></tr>
		<tr><td>Озвучивание:</td><td><textarea name="voice" rows="5" class="w98p"><?= persons_h($person['voice']) ?></textarea></td></tr>
		<tr><td>Продюсер:</td><td><textarea name="producer" rows="5" class="w98p"><?= persons_h($person['producer']) ?></textarea></td></tr>
		<tr><td>Режиссер:</td><td><textarea name="director" rows="5" class="w98p"><?= persons_h($person['director']) ?></textarea></td></tr>
		<tr><td>Сценарист:</td><td><textarea name="writer" rows="5" class="w98p"><?= persons_h($person['writer']) ?></textarea></td></tr>
		<tr><td>Награды:</td><td><textarea name="awards" rows="6" class="w98p"><?= persons_h($person['awards']) ?></textarea></td></tr>
		<tr><td>Ссылки:</td><td><textarea name="links" rows="5" class="w98p" placeholder="Название|https://example.com"><?= persons_h($person['links']) ?></textarea></td></tr>
		<tr><td>Источник:</td><td><input type="text" name="source_url" class="w98p" value="<?= persons_h($person['source_url']) ?>"></td></tr>
