...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_POST['action'] ?? '') === 'addtorrent') {
	...
	kz_groups_log($id, (int)$CURUSER['id'], 'torrent', 'Добавлена раздача #' . $torrent_id);
...
if (($_GET['action'] ?? '') === 'remove' && $can_manage) {
	...
	if ($torrent_id > 0) {
		...
		kz_groups_log($id, (int)$CURUSER['id'], 'torrent', 'Удалена раздача #' . $torrent_id);
		kz_groups_refresh_counts($id);
	}
	header('Location: /groupextorrents.php?id=' . $id);
	exit;
...
$rows = kz_groups_torrent_rows($id, $page * 60, 60, 'date');
	...
	<div class="mn3_content">
		...
		<div class="bx1">
			...
			<?php foreach ($rows as $row) { ?>
				<a href="/details.php?id=<?= (int)$row['id'] ?>" class="thumbnail" title="<?= kz_groups_h($row['name']) ?>">
					<img src="<?= kz_groups_torrent_poster($row) ?>" alt="" style="width:100px; height:140px; object-fit:cover;">
					<span class="small"><?= kz_groups_h(kz_groups_cut($row['name'], 38)) ?></span>
				</a>
			<?php } ?>
			<div class="clr"></div>
		</div>
