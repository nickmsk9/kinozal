...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageLoading:			'lightbox/loading.gif',		// (string) Path and the name of the loading icon
			imageBtnPrev:			'lightbox/prev.gif',			// (string) Path and the name of the prev button image
			imageBtnNext:			'lightbox/next.gif',			// (string) Path and the name of the next button image
			imageBtnClose:			'lightbox/close.gif',		// (string) Path and the name of the close btn
			...
			txtImage:				'Скриншот',	// (string) Specify text "Image"
		 ...
		 *
		 */
		function _show_image_data(intImageWidth,intImageHeight) {
			$('#lightbox-container-image-data-box').slideDown('fast');
			$('#lightbox-image-details-caption').hide();
			if ( settings.imageArray[settings.activeImage][1] ) {
				$('#lightbox-image-details-caption').html(settings.imageArray[settings.activeImage][1] + ' (' + intImageWidth + 'x' + intImageHeight + ')').show();
			}
